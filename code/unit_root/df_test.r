...
print(plot_series(x, "AR(2) simulation", "x_t"))
print(plot_series(dx, "First difference", "Delta x_t", "#e76f51"))

print(ggAcf(x, lag.max = 24) + labs(title = "AR(2) simulation - ACF") + plot_theme)
print(ggPacf(x, lag.max = 24) + labs(title = "AR(2) simulation - PACF") + plot_theme)
...
for (n in 1:N) {
  ...
  # Start at unconditional mean plus scaled innovation.
